...
} from '#src/valibot/internationalized-date/schema/internationalized-calendar-date-time';
import { internationalizedCalendarDateTime } from '#src/valibot/internationalized-date/schema/internationalized-calendar-date-time';

describe('internationalizedCalendarDateTime', () => {
  describe('should return schema object', () => {
    const baseSchema: Omit<InternationalizedCalendarDateTimeSchema<never>, 'message'> = {
      kind: 'schema',
    ...
    test('with undefined message', () => {
      const schema: InternationalizedCalendarDateTimeSchema<undefined> = { ...baseSchema, message: undefined };
      expect(internationalizedCalendarDateTime()).toStrictEqual(schema);
      expect(internationalizedCalendarDateTime(undefined)).toStrictEqual(schema);
    });

    test('with string message', () => {
      expect(internationalizedCalendarDateTime('message')).toStrictEqual({
        ...baseSchema,
        message: 'message',
      } satisfies InternationalizedCalendarDateTimeSchema<string>);
    });
    ...
    test('with function message', () => {
      ...
      expect(internationalizedCalendarDateTime(message)).toStrictEqual({
        ...baseSchema,
        message,
      } satisfies InternationalizedCalendarDateTimeSchema<typeof message>);
    });
  });
  ...
  describe('should return dataset without issues', () => {
    ...
    test('for a calendar date-time', () => {
      const value = new CalendarDateTime(2024, 1, 1, 10, 30, 0);
      expect(schema['~run']({ value }, {})).toStrictEqual({ typed: true, value });
    });

    test('for midnight', () => {
      const value = new CalendarDateTime(2024, 1, 1, 0, 0, 0);
      expect(schema['~run']({ value }, {})).toStrictEqual({ typed: true, value });
    });
  });

  describe('should return dataset with issues', () => {
    const schema = internationalizedCalendarDateTime('message');
    const baseIssue: Omit<InternationalizedCalendarDateTimeIssue, 'input' | 'received'> = {
      kind: 'schema',
      type: 'internationalized_calendar_date_time',
    ...
    test('for calendar date-time strings', () => {
      const str = '2024-01-01T10:30:00';
