...
import type { ToPlainDateAction } from '#src/valibot/internationalized-date/actions/to-plain-date-value';
import { toPlainDate } from '#src/valibot/internationalized-date/actions/to-plain-date-value';
...
describe('should transform to Temporal.PlainDate', () => {
  ...
  test('converts a ZonedDateTime with UTC timezone', () => {
    const value = new ZonedDateTime(2024, 1, 15, 'UTC', 0, 10, 0, 0);
    expect(action['~run']({ typed: true, value }, {})).toStrictEqual({
      typed: true,
      value: Temporal.ZonedDateTime.from(value.toString()).toPlainDate(),
    });
  });
  ...
  test('converts a CalendarDateTime', () => {
    const value = new CalendarDateTime(2024, 6, 15, 10, 0, 0);
    expect(action['~run']({ typed: true, value }, {})).toStrictEqual({
      typed: true,
      value: Temporal.PlainDateTime.from(value.toString()).toPlainDate(),
    });
  });
  ...
  test('converts a CalendarDate', () => {
    ...
    expect(action['~run']({ typed: true, value }, {})).toStrictEqual({
      typed: true,
      value: Temporal.PlainDate.from(value.toString()),
    });
  });
...
describe('should return dataset with issues', () => {
  ...
  test('for Temporal.PlainDateTime', () => {
    const value = Temporal.PlainDateTime.from('2024-03-20T09:00:00');
    expect(action['~run']({ typed: true, value }, {})).toStrictEqual({
      typed: false,
      value,
      issues: [{ ...baseIssue, input: value, received: '"Invalid conversion option"' }],
